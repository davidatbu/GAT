...
@pytest.fixture(scope="session")
def bert_embedder() -> layers.BertEmbedder:
...
class TestEmbedders:
    def test_basic(self, vocab_setup: VocabSetup, device: torch.device) -> None:
    ...
    def test_pos(self, vocab_setup: VocabSetup, device: torch.device) -> None:
        lssent = ["huh hoh", "something"]
        lslstok_id = vocab_setup.basic_vocab.batch_tokenize_and_get_tok_ids(lssent)
